...
mTaylor <- function(expr, ...){
	# packages
  if( !require(mosaic)) stop("Must load mosaic package.")
  if( !require(manipulate) ) 
		stop("Must use a manipulate-compatible version of R, e.g. RStudio")
	# functions
	vals <- list(...)
	f <- mosaic::makeFun(expr, ..., strict.declaration=FALSE )
  varname <- all.names(rhs(expr))
  if( length(varname) > 1 ) stop("Function of > 1 variable.")
  xlim <- inferArgs( vars=varname, dots=vals, defaults=list(xlim=c(0,1)) )[[1]]
	...
	dd <- list()
  for (k in 1:10) dd[[k]] <- mosaic::symbolicD(expr,..., strict.declaration=FALSE, .order=k)
	...
	myplot <- function(a, TaylorBeTrue, lsquares, xwid, n, err, which.plot){
		...
		x <- seq(min(xlim),max(xlim), length = 1000)
    ylab = paste("f(",varname,")",sep="")
    
		if(which.plot =="Plot Functions"){
			xyplot(f(x)~x, xlim = c(min(x), max(x)), ylab=ylab,xlab=varname, panel = mypanel)
		}
		else{
			xyplot(f(x)~x, xlim = c(min(x), max(x)), ylab=ylab,xlab=varname, panel = errpanel, ylab="Error")
		}
